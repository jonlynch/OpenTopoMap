...
function init_function()
	...
	csv_path = "data/saddle_directions.csv"
	f = io.open(csv_path, "r")
	if f then
		f:read("*line") -- skip header row
		local n = 0
		for line in f:lines() do
			local id_s, dir_s = line:match("^(%d+),(%d+)")
			if id_s then
				saddle_directions[id_s] = tonumber(dir_s)
				n = n + 1
			end
		end
		f:close()
		print("Loaded " .. n .. " saddle directions from " .. csv_path)
	end
end
...
function process_pois(polygon)
	...
	elseif natural == "saddle" or natural == "col" or natural == "notch" then
		type_tag = "saddle"
		mz = 12
		local dem_dir = saddle_directions[tostring(Id())]
	...
	if is_peak_type and mz < 11 then
		...
		if type(direction) == "number" then
			AttributeNumeric("direction", direction)
		elseif direction ~= nil and direction ~= "" then
